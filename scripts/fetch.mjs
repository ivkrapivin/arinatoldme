...
function main() {
  ...
  if (failures.length) {
    ...
    // Раньше здесь стоял успешный выход, и провал выкачки выглядел как удачный
    // прогон: новое видео просто не появлялось на сайте. Молчаливый сбой заметить
    // труднее громкого, поэтому валим прогон — уже скачанные видео при этом
    // сохранены и попадут на сайт следующей сборкой.
    process.exitCode = 1;
  }
}
