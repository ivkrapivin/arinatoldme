...
YouTube блокирует запросы с серверов GitHub («Sign in to confirm you're not a bot»),
поэтому выкачка новых видео делается с домашнего интернета:

```bash
cd ~/arina-site && npm run sync
```

Скрипт подтянет репозиторий, скачает новые видео с субтитрами и отправит их в GitHub.
Дальше всё делает Actions: редактирует расшифровки, собирает и публикует сайт.
Ключ Anthropic локально не нужен — редактура идёт в облаке.

Сигнал, что пора запускать: **красный прогон** еженедельного крона. Если новых видео
нет, прогон зелёный; если появилось и не скачалось — красный, и GitHub присылает письмо.
